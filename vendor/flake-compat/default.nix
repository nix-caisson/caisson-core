# VENDORED (MIT, see COPYING) from flake-parts vendor/flake-compat
# (github:hercules-ci/flake-parts rev 17c9d6cdfc60c64f4ee8d306f9bc0b4ccb51481e),
    ...
    (
      key: node:
        let
          ...
          # Import the root's flake.nix via the ORIGINAL path value when it is
          # already a store (sub)path: path + string stays a path, read
          # directly through the evaluator as a subpath of the (already valid)
          ...
          # `src` (the classic impure default.nix use), where importing the
          # live tree instead of the snapshot copy would change semantics.
          srcIsStorePath =
            builtins.isPath src
            && builtins.substring 0 (builtins.stringLength builtins.storeDir) (toString src)
               == builtins.storeDir;

          flakeNixPath =
            if key == lockFile.root && srcIsStorePath then
              src + "/flake.nix"
            else
              outPath + "/flake.nix";

          flake = import flakeNixPath;
